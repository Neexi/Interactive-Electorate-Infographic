...
addCard = function(data, x, y, width, height) {
	if(cardList.length < 7 && dupCheck(data) == 0) {
		cardList.push(data);
		makeCard(data, x, y, width, height);
...
Raphael.fn.makeMap = function (xMap,yMap, x, y, width, height){
    ...
    for (var i = map_data.length - 1; i >= 0; i--) {
        ...
        current_electorate.dblclick(function() {
		   addCard(getElectorate(this.data("electorate")), x, y, width, height);
        });
    };
...
makeCard = function(data, x, y, width, height) {
