...
Raphael.fn.makeMap = function (xMap,yMap, x, y, width, height){
    ...
    for (var i = map_data.length - 1; i >= 0; i--) {
        ...
        var current_electorate = this.path(path_value).attr({fill: color, "fill-opacity": 1});
    ...
    var container = this.rect(220, 720, 400, 50).attr({fill : '#00CC66', "fill-opacity": 1});
    var labour = this.rect(220,720,((90*400)/150),50).attr({fill : '#B25D5A', "fill-opacity": 1});
    var liberal = this.rect(220+((90*400)/150),720,((55*400)/150),50).attr({fill : '#4D4E8F', "fill-opacity": 1});
    ...
    reset.click(function() {
            for (var i = map_set.length - 1; i >= 0; i--) {
                ...
                map_set[i].attr({fill: color, "fill-opacity": 1});
            };
        });
...
Raphael.fn.makeBar = function(data, party, x, y, width, height){
	...
	fin = function () {
        sum_val = this.data("totalSum");
		this.flag = canvasBar.popup(this.bar.x, this.bar.y, (Math.round(this.bar.value*100/sum_val) + "%") || "0").insertBefore(this);
	}
	fout = function () {
	...
	[num], { colors: [result] });
    barChart.data("totalSum", totalSum);
